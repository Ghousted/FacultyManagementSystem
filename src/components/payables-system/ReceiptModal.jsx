...
const ReceiptLayout = ({ label, receiptData }) => {
  ...
  return (
    <div className="p-6 border border-slate-300 bg-white text-[12px] relative receipt-copy rounded-xl shadow-sm">
  ...
  />
      ...
      <div className="relative z-10 space-y-2">
        ...
        <div className="space-y-1.5">
          <div className="flex items-start justify-between gap-2">
            <div className="flex flex-col gap-1">
              <span className="border text-center py-0.5 rounded-full text-[11px] font-bold ">
                {label}
              </span>
              <p className="text-[10px] text-slate-500 tracking-wide font-semibold">OFFICIAL RECEIPT</p>
            </div>
            <div className="text-center flex-1 ">
              <h2 className="text-base font-bold text-blue-900 tracking-wide leading-tight">COLLEGE OF COMPUTER STUDIES</h2>

            </div>
            <div className="text-right whitespace-nowrap">

              <p className="text-[13px] font-bold text-slate-800">
                <span className="text-[12px] text-slate-500 font-semibold mr-1">RECEIPT NO.</span> {receiptNumber}
              </p>
        ...
        <div className="grid grid-cols-3 pt-1  border-b ">
          <div className="col-span-2 flex gap-2">
            <span className="text-sm uppercase tracking-wider text-slate-700 font-semibold min-w-fit">Name:</span>
            <span className="text-sm uppercase font-semibold text-slate-900 flex-1 wrap-break-word">
              {studentName}
            </span>
          </div>
          <div className="col-span-1 flex gap-2">
            <span className="text-sm uppercase tracking-wider text-slate-700 font-semibold min-w-fit">Course / Year:</span>
            <span className="text-sm uppercase font-semibold text-slate-900">
              {course || 'BSCS'} {yearLevel || ''}
            </span>
          </div>
        </div>

        <div className="border rounded-lg overflow-hidden my-2.5">
          <table className="w-full text-[10px]">
            <thead className="bg-slate-100">
              <tr>
                <th className="px-2 py-1 text-left font-semibold uppercase tracking-wide text-slate-600 text-sm">Description</th>
                <th className="px-2 py-1 text-right font-semibold uppercase tracking-wide text-slate-600 text-sm">Price</th>
                <th className="px-2 py-1 text-right font-semibold uppercase tracking-wide text-slate-600 text-sm">Prev. Bal.</th>
                <th className="px-2 py-1 text-right font-semibold uppercase tracking-wide text-slate-600 text-sm">Payment</th>
                <th className="px-2 py-1 text-right font-semibold uppercase tracking-wide text-slate-600 text-sm">Current Bal.</th>
              </tr>
            </thead>
            <tbody>
              {receiptItems.map((item) => (
                <tr key={item.key} className="border-t">
                  <td className="px-2 py-1.5 text-slate-800 text-sm">{item.description}</td>
                  <td className="px-2 py-1.5 text-right text-slate-900 font-semibold text-sm">{formatPhp(item.price)}</td>
                  <td className="px-2 py-1.5 text-right text-slate-700 font-semibold text-sm">{formatPhp(item.previousBalance)}</td>
                  <td className="px-2 py-1.5 text-right text-emerald-700 font-semibold text-sm">{formatPhp(item.payment)}</td>
                  <td className="px-2 py-1.5 text-right text-slate-900 font-bold text-sm">{formatPhp(item.balance)}</td>
                </tr>
              ))}
              {receiptItems.length > 1 && (
                <tr className="border-t bg-slate-50">
                  <td className="px-2 py-1.5 text-slate-800 font-bold text-sm">TOTAL</td>
                  <td className="px-2 py-1.5 text-right text-slate-900 font-bold text-sm">{formatPhp(totalPriceNum)}</td>
                  <td className="px-2 py-1.5 text-right text-slate-700 font-bold text-sm">{formatPhp(totalPreviousBalanceNum)}</td>
                  <td className="px-2 py-1.5 text-right text-emerald-700 font-bold text-sm">{formatPhp(totalPaymentAmountNum)}</td>
                  <td className="px-2 py-1.5 text-right text-slate-900 font-bold text-sm">{formatPhp(totalCurrentBalanceNum)}</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="border  rounded-lg overflow-hidden">
          <div className="px-2 py-0.5 bg-slate-100 border-b">
            <p className="text-sm uppercase tracking-wide font-semibold text-slate-600">
            ...
            <>
              ...
              <div className="grid grid-cols-2">
                {otherPayablesList.slice(0, 4).map((item) => (
                  <div
                    key={item.payableId}
                    className="flex items-center justify-between px-2 py-0.5 border-b odd:border-r "
                  >
                    <p className="text-sm text-slate-700 truncate pr-2">
                      {item.type}
                    </p>
        ...
        <div className="space-y-1 ">
          <div className='flex items-center gap-2'>
            <p className="text-sm tracking-wider text-slate-600 font-semibold">Mode of Payment:</p>
        ...
        {showReference && (
          <div className="space-y-1 flex items-center gap-2">
            <p className="text-sm tracking-wider text-slate-600 font-semibold">Reference Number:</p>
            <p className="text-sm font-semibold text-slate-900">{reference || 'N/A'}</p>
          </div>
        )}
        ...
        <div className="">
          <div className="w-46 text-center">
            <div className="h-4 border-b mb-1" />
            <p className="text-sm font-semibold text-slate-600 uppercase tracking-wide">Received By</p>
          </div>
        </div>
        </div>
